...
void communicationTask(void *args){
  ...
  while (1)
  {
    ...
    // debug_printf("communicationTask");
    /* 发送遥控器数据 */
...
void displayTask(void *args){
  debug_printfln("Display task started");

  uint32_t preTime = xTaskGetTickCount();
  while (1){
    debug_printfln("Display task...");
    App_Display_Show();
    vTaskDelayUntil(&preTime, DISPLAY_CYCLE);
  }
}
